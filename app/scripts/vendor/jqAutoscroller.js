...
(function($) {
    ...
    var k = "$autoscroll",
        ek = "$tAutoscroll",
        ik = "$autoscrollInterval",
        lk = "$edgeAutoscroll",
        mk = "$lastEdgeAutoscroll",
    ...
    // available M defaults to init
        M = {
            ...
            toggle: function() {
                return this.each(function() {
                    var c = d(this);
                    if (c) if (c.scroll) $(this).autoscroll("pause");
            ...
            addpausesource: function(e) {
                ...
                if (!(e instanceof jQuery)) if ((e = $(e)).length == 0) return this;
                var s = this.selector;
                e.each(function() {
                    $(this).hover(function() {
                        $(s).each(function() {
    ...
    // validation
        V = function(s) {
            if ($.inArray(typeof s.scroll, ["undefined", "boolean"]) < 0) $.error("scroll is not a boolean");
            if ($.inArray(typeof s.step, ["undefined", "number"]) < 0 && isNaN(s.step = Number(s.step))) $.error("step is not a valid number");
    ...
    // angle handler
        A = function(a) {
            if (typeof a === "string" && isNaN(Number(a))) if (G[a]) a = G[a];
